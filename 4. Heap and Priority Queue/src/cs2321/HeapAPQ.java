 ...
 * 2/22/2023
 * 
 * This is an implementation of an Adaptable PriorityQueue based on an heap.
 */
...
public class HeapAPQ<K,V> implements AdaptablePriorityQueue<K,V> {
    ...
    protected apqEntry<K,V> validate(Entry<K,V> entry)throws IllegalArgumentException{
        if(entry == null || !(entry instanceof apqEntry))
	...
	public Object[] data() {
		 ...
		 */
		// TODO: Uncomment the next 5 lines of code and replace variable data with the variable name of your array. 
		
		
		int size = data.size();
		datacopy=new Object[size];
		for (int i=0;i<size;i++) {
			datacopy[i]=data.get(i);
		}
